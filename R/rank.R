...
determineRank <- function(data, r.init = NULL, p, max.iter.lasso = 3, conv.lasso = 10^-2, max.iter.r = 5, beta.init, alpha.init, rho.glasso = 0.1, lambda.gamma, lambda_beta, intercept=FALSE, glmnetthresh = 1e-04) {
  Y <- data$diff
  X <- data$diff_lag
  Z <- data$level
  ...
  while ((iter < max.iter.r) & (diff.r > 0)) {
    ...
    if (r.init == 0) {
      ...
      lasso_fit <- rankSelectionCriterion(Y = Y, X = X, Z = Z, beta.init = beta.init.fit, alpha.init = alpha.init.fit, p = p, r = r.init, max.iter = max.iter.lasso, conv = conv.lasso, lambda.gamma = lambda.gamma, lambda_beta = lambda_beta, rho.glasso = rho.glasso, intercept=intercept, glmnetthresh = glmnetthresh)
...
rankSelectionCriterion <- function(p, Y, X, Z, r, alpha.init = NULL, beta.init, max.iter = 25, conv = 10^-3, lambda.gamma = 0.001, lambda_beta = matrix(seq(from = 2, to = 0.001, length = 100), nrow = 1), rho.glasso = 0.5, cutoff = 0.8, intercept = F, glmnetthresh = 1e-04) {
  # Dimensions
  q <- dim(Y)[2]
  n <- dim(Y)[1]
  ...
  while ((iter < max.iter) & (diff.obj > conv)) {
    ...
    gamma_fit <- nts.gamma.fixed.lassoreg(Y = Y, X = X, Z = Z, Pi = Pi.init, p = p, Omega = Omega.init, lambda.gamma = lambda.gamma, intercept=intercept, glmnetthresh = glmnetthresh)
    # Obtain alpha
    ...
    beta_fit <- nts.beta(Y = Y, X = X, Z = Z, zbeta = gamma_fit$zbeta, rank = r, P = gamma_fit$P, alpha = alpha_fit$alpha, alphastar = alpha_fit$alpha_star, lambda_grid = lambda_beta, rho.glasso = rho.glasso, cutoff = cutoff, intercept=intercept, glmnetthresh = glmnetthresh)
